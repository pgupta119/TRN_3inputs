...
class GeneralizedTRN(nn.Module):
    ...
    def encoder(self, camera_input, sensor_input, optical_input,future_input, enc_hx, enc_cx):
        fusion_input = self.feature_extractor(camera_input,sensor_input,optical_input)
    ...
    def step(self, camera_input, sensor_input,optical_input, future_input, enc_hx, enc_cx):
        # Encoder -> time t
        enc_hx, enc_cx, enc_score = \
                self.encoder(camera_input,sensor_input, optical_input,future_input, enc_hx, enc_cx)

        # Decoder -> time t + 1
        dec_score_stack = []
        dec_hx = self.hx_trans(enc_hx)
        dec_cx = self.cx_trans(enc_cx)
        fusion_input = camera_input.new_zeros((camera_input.shape[0], self.hidden_size)) + optical_input.new_zeros((optical_input.shape[0], self.hidden_size))
        future_input = camera_input.new_zeros((camera_input.shape[0], self.future_size)) + optical_input.new_zeros((optical_input.shape[0], self.future_size))
    ...
    def forward(self, camera_inputs, sensor_inputs,optical_inputs):
        batch_size = camera_inputs.shape[0]
        enc_hx = camera_inputs.new_zeros((batch_size, self.hidden_size)) + optical_inputs.new_zeros((batch_size, self.hidden_size))
        enc_cx = camera_inputs.new_zeros((batch_size, self.hidden_size)) + optical_inputs.new_zeros((batch_size, self.hidden_size))
        ...
        for enc_step in range(self.enc_steps):
            ...
            dec_hx = self.hx_trans(enc_hx)
            dec_cx = self.cx_trans(enc_cx)
            fusion_input = camera_inputs.new_zeros((batch_size, self.hidden_size)) + optical_inputs.new_zeros((batch_size, self.hidden_size))
            future_input = camera_inputs.new_zeros((batch_size, self.future_size)) + optical_inputs.new_zeros((batch_size, self.future_size))
            for dec_step in range(self.dec_steps):
                dec_hx, dec_cx, dec_score = self.decoder(fusion_input, dec_hx, dec_cx)
                dec_score_stack.append(dec_score)
                fusion_input = self.fusion_linear(dec_score)
