...
class HDDFeatureExtractor(nn.Module):
    def __init__(self, args):
        ...
        if args.inputs in ['camera', 'sensor', 'multimodal']:
            ...
            self.with_optical ='optical' not in args.inputs

        else:
            raise(RuntimeError('Unknown inputs of {}'.format(args.inputs)))

        if self.with_camera and self.with_sensor and self.with_optical :
            self.fusion_size = 1280 + 1280 + 20
        ...
        self.camera_linear = nn.Sequential(
            nn.Conv2d(1600, 20, kernel_size=1),
            nn.ReLU(inplace=True),
            Flatten(),
        )
        self.optical_linear = nn.Sequential(
            nn.Conv2d(1600, 20, kernel_size=1),
            nn.ReLU(inplace=True),
            Flatten(),
        )
    ...
    def forward(self, camera_input, sensor_input, optical_input):
        if self.with_camera:
            camera_input = self.camera_linear(camera_input)
        if self.with_sensor:
            sensor_input = self.sensor_linear(sensor_input)
        if self.with_optical:
            optical_input = self.optical_linear(optical_input)


        if self.with_camera and self.with_sensor and self.with_optical:
            fusion_input = torch.cat((camera_input ,sensor_input,optical_input), 1)
        elif self.with_camera:
...
class THUMOSFeatureExtractor(nn.Module):
    def __init__(self, args):
        ...
        if self.with_camera and self.with_motion:
            self.fusion_size = 2048 + 2048 + 1024
        elif self.with_camera:
