...
# /Users/prakashgupta/TRN.pytorch/_init_paths.py
import _init_paths
sys.path.append("/workspace/persistent/TRN.pytorch/lib/")
import utils as utl
sys.path.append("/workspace/persistent/TRN.pytorch/configs/hdd")
from trn_hdd import parse_trn_args as parse_args
    ...
    for epoch in range(args.start_epoch, args.start_epoch + args.epochs):
        ...
        for phase in args.phases:
            ...
            with torch.set_grad_enabled(training):
                ...
                for batch_idx, (camera_inputs, sensor_inputs, optical_inputs,enc_target, dec_target) \
                        in enumerate(data_loaders[phase], start=1):
                    batch_size = camera_inputs.shape[0]
                    camera_inputs = camera_inputs.to(device)
                    sensor_inputs = sensor_inputs.to(device)
                    optical_inputs = optical_inputs.to(device)
                    enc_target = enc_target.to(device).view(-1)
                    dec_target = dec_target.to(device).view(-1)

                    enc_score, dec_score = model(camera_inputs, sensor_inputs,optical_inputs)
